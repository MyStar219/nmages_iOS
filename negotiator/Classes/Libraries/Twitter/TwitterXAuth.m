...
- (NSString *) timestamp
{
    if (timestamp == nil) {
        ...
        ASIHTTPRequest *timeRequest = [ASIHTTPRequest requestWithURL:[NSURL URLWithString:@"https://www.twitter.com"]];
        [timeRequest startSynchronous];
        NSError *error = [timeRequest error];
        if (!error) {
            NSString *dateHeader = [[timeRequest responseHeaders] objectForKey:@"Date"];
            if (![NSString isNilOrEmpty:dateHeader]) {
                NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
                [dateFormatter setDateFormat:@"EEE, dd LLL yyyy HH:mm:ss z"];
                //Tue, 20 Dec 2011 15:50:26 GMT | 
                
                NSDate *twitterDate = [dateFormatter dateFromString:[dateHeader stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]]];
                [dateFormatter release];
                
                if (twitterDate) {
                    double difference = [twitterDate timeIntervalSinceDate:currentDate];
                    timestamp = [[NSString stringWithFormat:@"%.f", ([currentDate timeIntervalSince1970] + difference)] retain];
                    return timestamp;
                }
            }
        }
        
        timestamp = [[NSString stringWithFormat:@"%f", [[NSDate date] timeIntervalSince1970]] retain];
        return timestamp;
    } 
    return timestamp;
}
...
- (void) authorize
{
    ...
    [twitterURL release];
    twitterURL = [@"https://api.twitter.com/oauth/access_token" retain];
...
- (void) tweet:(NSString *)status
{
    ...
    [twitterURL release];
    twitterURL = [@"http://api.twitter.com/1/statuses/update.json" retain];
    NSMutableURLRequest* postRequest = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:twitterURL]];
    [postRequest setHTTPMethod: @"POST"];
    NSArray * parameterArray = [NSArray arrayWithObjects:
    ...
    [postRequest addValue:dateDescription forHTTPHeaderField:@"Date"];
